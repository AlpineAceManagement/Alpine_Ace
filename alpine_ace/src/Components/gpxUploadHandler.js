...
router.post("/api/upload", async (req, res) => {
    try {
        ...
        for (let i = 0; i < streckenpunkte.length - 1; i++) {
            ...
            const lon1_m = lon1_grad * 111319.9 * Math.cos(lat1_grad * Math.PI / 180);
            const lat2_m = lat2_grad * 111319.9;
            const lon2_m = lon2_grad * 111319.9 * Math.cos(lat2_grad * Math.PI / 180);

            // Calculer la distance 3D
            const distance = entfernung_3d(lat1_m, lon1_m, ele1, lat2_m, lon2_m, ele2);
            ...
            const geschwindigkeit_mps = zeitunterschied !== 0 ? distance / zeitunterschied : 0;
        ...
        const gesamtzeit_formatted = `${gesamtstunden.toString().padStart(2, "0")}:${gesamtminuten.toString().padStart(2, "0")}:${gesamtsekunden.toString().padStart(2, "0")}`;

        // Se connecter à la base de données PostgreSQL
        const pool = new Pool(config.db_config);

        // Extraire les données géométriques du fichier GPX et les convertir au format PostgreSQL
        const geometrie_text = streckenpunkte.map(point => `${parseFloat(point.$.lon)} ${parseFloat(point.$.lat)}`).join(",");
        ...
        // Envoyer une réponse
        res.status(200).json({ message: "GPX file uploaded and processed successfully." });
    } catch (error) {
